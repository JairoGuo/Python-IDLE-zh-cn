...
class Debugger:
    ...
    def make_gui(self):
        ...
        self.buttons = bl = []
        #
        self.bcont = b = Button(bframe, text="执行", command=self.cont)
        bl.append(b)
        self.bstep = b = Button(bframe, text="下一步", command=self.step)
        bl.append(b)
        self.bnext = b = Button(bframe, text="跳过", command=self.next)
        bl.append(b)
        self.bret = b = Button(bframe, text="跳出", command=self.ret)
        bl.append(b)
        self.bret = b = Button(bframe, text="退出", command=self.quit)
        bl.append(b)
        #
        for b in bl:
        ...
        self.bstack = Checkbutton(cframe,
            text="栈", command=self.show_stack, variable=self.vstack)
        self.bstack.grid(row=0, column=0)
        if not self.vsource:
            self.__class__.vsource = BooleanVar(top)
        self.bsource = Checkbutton(cframe,
            text="源", command=self.show_source, variable=self.vsource)
        self.bsource.grid(row=0, column=1)
        if not self.vlocals:
            self.__class__.vlocals = BooleanVar(top)
            self.vlocals.set(1)
        self.blocals = Checkbutton(cframe,
            text="局部", command=self.show_locals, variable=self.vlocals)
        self.blocals.grid(row=1, column=0)
        if not self.vglobals:
            self.__class__.vglobals = BooleanVar(top)
        self.bglobals = Checkbutton(cframe,
            text="全局", command=self.show_globals, variable=self.vglobals)
    ...
    def show_locals(self):
        ...
        if self.vlocals.get():
            if not lv:
                self.localsviewer = NamespaceViewer(self.flocals, "局部")
        else:
            if lv:
    ...
    def show_globals(self):
        ...
        if self.vglobals.get():
            if not gv:
                self.globalsviewer = NamespaceViewer(self.fglobals, "全局")
        else:
            if gv:
